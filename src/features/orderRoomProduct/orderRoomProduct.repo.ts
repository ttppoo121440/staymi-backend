    ...
    const { data, pagination } = await this.paginateQuery<{
      ...
      (limit, offset) =>
        db
          ...
          .from(order_room_product)
          .leftJoin(hotels, eq(order_room_product.hotel_id, hotels.id))
          .leftJoin(room_plans, eq(order_room_product.room_plans_id, room_plans.id))
          .leftJoin(hotel_rooms, eq(room_plans.hotel_room_id, hotel_rooms.id))
          .leftJoin(room_types, eq(hotel_rooms.room_type_id, room_types.id))
          .leftJoin(order_room_product_item, eq(order_room_product_item.order_id, order_room_product.id))
          .leftJoin(product_plans, eq(order_room_product_item.product_plans_id, product_plans.id))
          .leftJoin(products, eq(product_plans.product_id, products.id))
          .where(and(...conditions))
          .limit(limit)
          .offset(offset),
      async () => {
